  ...
  async (req: Request, res: Response) => {
    try {
      ...
      INSERT INTO users (
        ...
        phone, employee_id, student_id, is_active, approval_status
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, true, 'pending')
    `,
        [
          fullName,
          email,
          hashedPassword,
          roleId || 4, // Default to student role
          organizationId || null,
          null, // phone
          employeeId || null,
          studentId || null,
    ...
    if (user.role_name !== "super_admin") {
      if (user.approval_status === "pending") {
        return res.status(403).json({
          error:
            "Your account is pending admin approval. Please wait for validation.",
          status: "pending",
        });
      }
...
router.get("/me", authenticateJWT, async (req: Request, res: Response) => {
  try {
    ...
    const userResult = await query(
      `
      SELECT u.*, r.name as role_name, r.description as role_description, o.name as organization_name, o.code as organization_code
      FROM users u 
      LEFT JOIN roles r ON u.role_id = r.id 
    ...
    console.log("GET /me - Query result:", userResult.rows.length, "rows");
